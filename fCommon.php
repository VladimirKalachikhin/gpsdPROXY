...
elseif(in_array($socket,$masterSocks,true)){ 	// умерло входное подключение
	...
	if($n !== FALSE){
		unset($sockets[$n]);
		if($messages[$n]['PANOrole']) $action = 'PANOrole';
		unset($messages[$n]);
		$pollWatchExist = array();	// очистим список сокетов с режимом WATCH
		foreach($messages as $ind => $sockData){	// для каждого сокета
	...
	if($action == 'PANOrole'){	// разошлём подписчикам оповещение об изменении списка. Свежий список подписчиков уже есть.
...
if($pollWatchExist){	// есть режим WATCH, надо подготовить данные. От gpsd (или что там вместо) может прийти пустое или непонятное
	...
	foreach($updatedTypes as $updatedType => $v){
		...
		case "WPT":
			...
			break;
		case 'PANO':
			// Здесь данные не есть состояние сервера, а просто передаются через.
			// Поэтому используется instrumentsDataUpdated, где key - как и для всего остального,
			// а value - для данных. Во всех остальных случаях value = true.
			$PANO = json_encode($instrumentsDataUpdated['PANO'], JSON_PRESERVE_ZERO_FRACTION | JSON_UNESCAPED_UNICODE)."\r\n\r\n";	// так как PANO не есть состояние сервера, а только сообщение, пересылаемое между клиентами, оно хранится непосредственно в $instrumentsDataUpdated
			//echo "\n [updAndPrepare] prepare PANO data to send=$PANO";
			break;
		};
	};
	...
	foreach($messages as $socket => $sockData){	// для каждого сокета
		...
		foreach($sockData['subscribe'] as $subscribe=>$v){
			...
			case "WPT":
				...
				break;
			case "PANO":
				// Если сообщение адресное - отошлём только адресату
				if($instrumentsDataUpdated['PANO']['clientToName'] and ($instrumentsDataUpdated['PANO']['clientToName'] != $sockData['clientName'])) break;
				$messages[$socket]['output'][] = $PANO;
				//echo "sending PANO to socket=$socket;                    \n";
				break;
			};
		};
	};
